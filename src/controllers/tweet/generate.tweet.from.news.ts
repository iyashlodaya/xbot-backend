...
const generateTweetFromTrendingTopic = async (
  req: Request,
  res: Response
): Promise<void> => {
  ...
  const tweetPrompt = `Summarize the following news article in a ${tone} tone, Keeping it ${
    lengthMap[length] || lengthMap["short"]
  }. Ensure the tweet is in ${language}. Include relevant hashtags and emojis based on the content. ${
    hashtags ? `Include relevant hashtags.` : `Do not include hashtags.`
  }. 
                        ${
                          emoji
                            ? `Use engaging emojis where appropriate.`
                            : `Avoid using emojis.`
                        }.  
                        News Article: "${title}"
                        News Description: "${description}"
                        Ensure the tweet is engaging, concise, and shareable.
                        `;
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a social media expert generating engaging tweets.",
        },
...
export default generateTweetFromTrendingTopic;
